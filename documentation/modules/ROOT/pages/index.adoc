= Welcome to the ARO ILT!

:page-layout: home
include::include_vars.adoc[]
:!sectids:

Welcome to your working ARO ILT environment with a guid of {guid}

You can access it via `ssh` with the following command:

[source, java,subs="+macros,+attributes"]
----
ssh command is {bastion_ssh_command}
----

And your password is {ssh_password}

Your Passowrd, {bastion_password}, will also work with your Visual Studio Code session


[.text-center.strong]
== Publish Services to Kubernetes

A Kubernetes tutorial to show how you can deploy a Java service to a Kubernetes cluster as it was a child game.

[.tiles.browse]
== Browse modules

[.tile]
.xref:01-setup.adoc[Get Started]
* xref:01-setup.adoc#minikube[Minikube]

[.tile]
.xref:02-deploy.adoc[Deploying]
* xref:02-deploy.adoc#package[Package the Application]
* xref:02-deploy.adoc#deploy[Deploy the Application]